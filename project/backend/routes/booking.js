...
app.post('/ticket', (req, res) => {
    ...
    const { UserId, TrainNo, NoOfPassengers, BookingDate, TotalFare } = req.body;

    const bookingQuery = `
        INSERT INTO booking (UserId, TrainNo, NoOfPassengers, BookingDate, TotalFare) 
        VALUES (?, ?, ?, ?, ?)
    `;

    connection.query(
        bookingQuery,
        [UserId, TrainNo, NoOfPassengers, BookingDate, TotalFare],
        (error, result) => {
            res.setHeader("content-type", "application/json");
            if (!error) {
                res.status(201).json({
